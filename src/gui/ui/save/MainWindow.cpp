...
#endif
#ifdef DEBUG
	std::cerr << "MainWindow initialized.\n";
#endif
...
void MainWindow::FileImport()
{
	...
	{
		const Glib::ustring filename( filedlg.get_filename() );
		...
		if( ret_val == 0 )
		{
			...
			else
			{
				...
				{
					Glib::ustring message( gettext("Cannot import Turn Watcher legacy file! The file must be corrupted.") );
...
void MainWindow::EditDuplicate()
{
	...
	if( response_id == Gtk::RESPONSE_OK )
	{
		...
		int char_num = 1;
		Glib::ustring base_name( sel_ch->name() );
		const int space = base_name.find_last_of(' ');
		if( space != Glib::ustring::npos )
		{
			const Glib::ustring last_word( base_name.substr( space ) );
...
{
	const Glib::ustring	event( "Roll::" + stat->name() );
	//
...
void MainWindow::FillMainBox()
{
	...
	show_all_children();
#ifdef DEBUG
	std::cerr << "MainWindow has its main box filled.\n";
#endif
}
...
#ifdef DEBUG
	std::cerr  << "OnCharListChanged() " << std::endl;
#endif
	UpdateUI( false );
...
{
#ifdef DEBUG
	std::cerr  << "OnDoubleClick() " << std::endl;
#endif
...
bool MainWindow::OnNagEvent()
{
	...
	{
#ifdef DEBUG
		std::cerr << "NAG MINS: mins_since_start: " << mins_since_start << std::endl;
#endif
	...
	{
#ifdef DEBUG
		std::cerr << "NAG_QUIT_MINS: mins_since_start: " << mins_since_start << std::endl;
#endif
		hide();
	}
...
{
#ifdef DEBUG
	std::cerr << "MainWindow::on_hide()" << std::endl;
#endif
