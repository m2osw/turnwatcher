...
void EffectsEditor::AddEffects( Combatant::Character::Pointer ch )
{
	f_treeView.AddEffects( ch );
}


void EffectsEditor::GetEffects( Combatant::Character::Pointer ch )
{
...
void EffectsEditor::OnEditButton( const bool add )
{
	...
	while( 1 )
	{
		...
		if( response_id == Gtk::RESPONSE_OK )
		{
			...
			if( dups )
			{
				Glib::ustring message = gettext("Duplicate effect name--use another one!");
