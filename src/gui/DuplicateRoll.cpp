...
void DuplicateRoll::AddCharacter ( Combatant::Character::Pointer ch )
{
...
bool DuplicateRoll::FindCharacter( Combatant::Character::Pointer ch )
{
	...
	{
#if defined(DEBUG) && defined(TRACE)
		//printf( "[Single character in roll object], " );
#endif
		const molib::moWCString fname = f_char->name();
		const molib::moWCString cname = ch->name();
	...
	{
#if defined(DEBUG) && defined(TRACE)
		//printf( "[More than one character in roll object], " );
#endif
...
void DuplicateRoll::RemoveCharacter( Combatant::Character::Pointer ch )
{
#if defined(DEBUG) && defined(TRACE)
	...
	{
#if defined(DEBUG) && defined(TRACE)
		//printf( "[More than one character in resolver], " );
#endif
...
#if defined(DEBUG) && defined(TRACE)
			printf( "DuplicateRoll::RemoveCharacter(): Searching for character %s: ", ch->name().c_str() );
			//printf( "[Resolver is now empty, falling back to single character %s]\n", f_char->name().c_str() );
...
void DuplicateRoll::GetOrderedVector( Combatant::Character::List& charList )
{
	charList.clear();
...
void DuplicateRoll::Copy( DuplicateRoll::Pointer dup_roll )
{
...
void DuplicateRoll::Load( molib::moPropBagRef& propBag )
{
	molib::moPropStringRef	singleChar ( "CHARACTER" );
	singleChar.Link ( propBag );
	if ( singleChar.HasProp() )
	{
		molib::moWCString				name( singleChar );
		Combatant::Character::Pointer	ch  ( Application::Manager::Instance().lock()->GetCharacterMgr().lock()->FindCharacter( name ) );
#if defined(DEBUG) && defined(TRACE)
		printf( "DuplicateRoll::Load() - searching for character %s, ", name.c_str() );
		if( ch )
		{
			printf( "found: %p\n", (Combatant::Character*) ch );
		}
		else
		{
...
void DuplicateRoll::Save( molib::moPropBagRef& propBag )
{
	if( f_char && !f_dupResolver )
	{
		molib::moPropStringRef	singleChar ( "CHARACTER" );
		singleChar.NewProp();
		singleChar = f_char->name();
		propBag += singleChar;
	}
	else if( f_dupResolver )
	{
