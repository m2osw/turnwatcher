...
namespace
{
	...
	const molib::moWCString	PublicName	("PublicName");
	const molib::moWCString	Type		("Type");
	const molib::moWCString	Initiative	("Initiative");
	const molib::moWCString	HitPoints	("HitPoints");
	const molib::moWCString	TempHP		("TempHP");
	const molib::moWCString	Damage		("Damage");
	const molib::moWCString	Level		("Level");
#else
	const molib::moWCString	Name		("Name");
	const molib::moWCString	Type		("Type");
	const molib::moWCString	HitDice		("HitDice");
	const molib::moWCString	HitPoints	("HitPoints");
	const molib::moWCString	CharEffects	("CharEffects");
	const molib::moWCString	Initiative	("Initiative");
	const molib::moWCString	TempHP		("TempHP");
	const molib::moWCString	Damage		("Damage");
#endif
...
EditWindow::EditWindow( Glib::ustring title, Gtk::Window& parent )
...
{
	typedef std::vector<Attribute::Stat::Pointer> StatList;
	StatList statList;
	//
...
void EditWindow::AddStatEntry( const char* name, const molib::mo_name_t id )
{
...
void EditWindow::AddStatNotes( const char* name, const molib::mo_name_t id )
{
	Gtk::Entry* entry;
...
namespace
{
...
#ifdef DEBUG
		molib::moWCString name = id;
		std::cout << "CharacterListUI: setting " << name.c_str() << " to " << text.c_str() << std::endl;
#endif
	}

	int GetChoice( LabelTable& tbl, const molib::moName& id )
	{
		Gtk::ComboBoxText* c;
		tbl.GetItem( id, c );
	...
	void SetChoice( LabelTable& tbl, const molib::moName& id, const bool value )
	{
		Gtk::ComboBoxText* c;
		tbl.GetItem( id, c );
	...
	void GetNotes( LabelTable& tbl, const molib::moName& id, Combatant::Character& ch )
	{
		Gtk::Entry* e = (Gtk::Entry*) tbl.GetItem( id, 0 );
		ch.setNotes( id, e? e->get_text().c_str(): 0 );
	}

	void SetNotes( LabelTable& tbl, const molib::moName& id, Combatant::Character& ch )
	{
		Gtk::Entry* e = (Gtk::Entry*) tbl.GetItem( id, 0 );
		if( e ) e->set_text( ch.getNotes( id ).c_str() );
	}

	void GetStat( LabelTable& tbl, const molib::moName& id, Combatant::Character& ch )
	{
		Gtk::SpinButton* s = (Gtk::SpinButton*) tbl.GetItem( id, 0 );
		ch.setMod( id, s? atoi(s->get_text().c_str()): 0 );
	}

	void SetStat( LabelTable& tbl, const molib::moName& id, Combatant::Character& ch )
	{
		Gtk::SpinButton* s = (Gtk::SpinButton*) tbl.GetItem( id, 0 );
...
#if !defined(OLD_UI)
	void GetSave( LabelTable& tbl, const molib::moName& id, SaveStat* save )
	{
		assert(save);
	...
	void SetSave( LabelTable& tbl, const molib::moName& id, SaveStat* save )
	{
		assert(save);
		Gtk::SpinButton* b = (Gtk::SpinButton*) tbl.GetItem( id, 0 );
		Gtk::SpinButton* m = (Gtk::SpinButton*) tbl.GetItem( id, 1 );
	...
	void GetSkill( LabelTable& tbl, const molib::moName& id, Skill* skill )
	{
		assert(skill);
	...
	void SetSkill( LabelTable& tbl, const molib::moName& id, Skill* skill )
	{
		Gtk::SpinButton* r = (Gtk::SpinButton*) tbl.GetItem( id, 0 );
		Gtk::SpinButton* m = (Gtk::SpinButton*) tbl.GetItem( id, 1 );
...
#if defined(OLD_UI)
	molib::moWCString name = f_char->name();
	SetText  ( f_baseTable, Name,       name.c_str() );
	//
	molib::moWCString pub = f_char->public_name();
	SetText  ( f_baseTable, PublicName, pub.c_str() );
	//
...
#else
	SetText  ( f_baseTable, Name,       f_char->name().c_str() );
	SetChoice( f_baseTable, Type,       f_char->monster() );
...
void EditWindow::UpdateCharFromDlg()
{
	...
	molib::moWCString char_name( GetText( f_baseTable, Name ).c_str(), -1, molib::mowc::MO_ENCODING_UTF8 );
	f_char->name   ( char_name );
	//
	molib::moWCString public_char_name( GetText( f_baseTable, PublicName ).c_str(), -1, molib::mowc::MO_ENCODING_UTF8 );
	f_char->public_name   ( public_char_name );
	//
	...
	//const Glib::ustring text = f_textBuffer->get_text();
	// by default, (char *) strings are taken as ISO-8859-1, here we need a UTF-8
	molib::moWCString text( f_textBuffer->get_text().c_str(), -1, molib::mowc::MO_ENCODING_UTF8 );
	f_char->notes( text );
...
void EditWindow::GetData( Combatant::Character::Pointer ch )
{
	if( !ch ) return;
	UpdateCharFromDlg();
	ch->Copy( f_char );
...
void EditWindow::SetData( Combatant::Character::Pointer ch )
{
	if( !ch ) return;
