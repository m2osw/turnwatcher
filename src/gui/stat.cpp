...
void Stat::id     		( const molib::mo_name_t val	)	{ f_id        		= val; 	f_statChanged.emit(); }
void Stat::abilityId	( const molib::mo_name_t val   	) 	{ f_abilityId 		= val;	f_statChanged.emit(); }
void Stat::name     	( const molib::moWCString& val 	)	{ f_name      		= val;	f_statChanged.emit(); }
...
void Stat::accel    	( const molib::moWCString& val 	)	{ f_accel     		= val;	f_statChanged.emit(); }
...
void Stat::Copy( const Pointer copy )
{
	f_id               = copy->f_id;
...
void Stat::Load( molib::moPropBagRef& propBag )
{
	molib::moPropStringRef	id        		( f_idName        		);
	molib::moPropStringRef	abilityId 		( f_abilityIdName  		);
	molib::moPropIntRef		legacyId  		( f_legacyIdName   		);
	molib::moPropIntRef		legacyType		( f_legacyTypeName 		);
	molib::moPropStringRef	name      		( f_nameName       		);
	molib::moPropIntRef		dice      		( f_diceName       		);
	molib::moPropIntRef		faces     		( f_facesName      		);
	molib::moPropIntRef		modifier  		( f_modifierName   		);
	molib::moPropIntRef		deleted  		( f_deletedName    		);
	molib::moPropStringRef	accel     		( f_accelName      		);
	molib::moPropIntRef		showOnToolbar	( f_showOnToolbarName	);
	molib::moPropIntRef		showOnHUD	 	( f_showOnHUDName		);
	molib::moPropIntRef		showMonsterOnHUD( f_showMonsterOnHUDName		);
	molib::moPropIntRef		internal		( f_internalName		);
	molib::moPropIntRef		ability		    ( f_abilityName			);
	molib::moPropIntRef		order			( f_orderName			);
	...
	if( id.HasProp() )
	{
		f_id 			= molib::moName( (molib::moWCString) id        );
	}
	...
	{
		throw molib::moError( "Bad Stat data for object!" );
	}

	if( abilityId.HasProp() )
	{
		f_abilityId = molib::moName( (molib::moWCString) abilityId );
	}
	else
	{
		f_abilityId = molib::moName( "UNNAMED" );
	}
	...
	if( f_legacyId != -1 )
	{
		...
		std::cout	<< "Legacy Id: " << f_legacyId 
					...
					<< ", NEW ID=" << molib::moWCString(molib::moName(f_id)).c_str()
					<< std::endl;
#endif
	}
...
void Stat::Save( molib::moPropBagRef& propBag )
{
	...
	order			.NewProp();
	//
	id        		= (molib::moWCString) molib::moName(f_id);
	abilityId 		= (molib::moWCString) molib::moName(f_abilityId);
	name      		= f_name;
	dice      		= f_dice;
	faces     		= f_faces;
	modifier  		= f_modifier;
...
void Value::Copy( const Pointer copy )
{
	f_stat	= copy->f_stat;
	f_mod	= copy->f_mod;
	f_roll	= copy->f_roll;
...
/// \sa Load
//
void Value::LoadLegacy( molib::moPropBagRef& propBag )
{
	// Figure out legacy stuff...we have to go by id and type
	//
	molib::moPropIntRef	legacy_id  ( "ID"   );
	molib::moPropIntRef	legacy_type( "TYPE" );
	//
	...
	if( !(legacy_id.HasProp() && legacy_type.HasProp()) )
	{
		...
		//
		throw molib::moError( "Cannot find legacy id and/or legacy value prop!" );
	}
	//
	auto statMgr = Application::Manager::Instance().lock()->GetStatMgr().lock();
	//
...
#endif
		throw molib::moError( "Cannot find legacy id and/or legacy value prop in StatManager!" );
	}
	assert( f_stat );
}
...
/// \sa LoadLegacy, Save
//
void Value::Load( molib::moPropBagRef& propBag )
{
	molib::moPropStringRef	stat( f_statName );
	molib::moPropIntRef		mod ( f_modName  );
	molib::moPropIntRef		roll( f_rollName );
	molib::moPropStringRef	notes( f_notesName );
	//
	stat.Link( propBag );
	mod .Link( propBag );
	roll.Link( propBag );
	...
	if( stat.HasProp() )
	{
		f_stat = Application::Manager::Instance().lock()->GetStatMgr().lock()->GetStat( molib::moName(stat) );
	}
	else
	{
		LoadLegacy( propBag );
...
//
/// \sa Load
void Value::Save( molib::moPropBagRef& propBag )
{
	molib::moPropStringRef stat  ( f_statName  );
	molib::moPropIntRef    mod   ( f_modName   );
	molib::moPropIntRef    roll  ( f_rollName  );
	...
	notes.NewProp();
	//
	stat = (molib::moWCString) molib::moName(f_stat->id());
	mod  = f_mod;
	roll = f_roll;
	notes = f_notes;
	//
