...
void InitWindow::InitTable()
{
...

	{
		...
		for( auto ch : f_chars )
		{
			...
			sp->set_numeric( true );
			const molib::mo_name_t init = ch->getRoll( initId );
			//
...
namespace
{
	bool lt_charcmp( Combatant::Character::Pointer c1, Combatant::Character::Pointer c2 )
	{
...
void InitWindow::SetCharacters( CharacterPairs& charPairList )
{
...
void InitWindow::OnAcceptButton()
{
...

	{
		...
		for( auto ch : f_chars )
		{
			...
			ss << "Character " << id << std::ends;
			molib::moName name( ss.str().c_str() );
			const int init = GetValue( f_charTable, name );
			assert(ch);
			ch->setRoll( initId, init );
		}
	}

	{
		...
		for( auto ch : f_monsters )
		{
			...
			ss << "Monster " << id << std::ends;
			molib::moName name( ss.str().c_str() );
			const int init = GetValue( f_monsterTable, name );
			assert(ch);
			ch->setRoll( initId, init );
		}
	}
