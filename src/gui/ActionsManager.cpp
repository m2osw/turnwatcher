...
void Desensitizer::PushAll()
{
	std::vector<motk::ActionPtr> actions
		= Application::Manager::Instance().lock()->GetActionsMgr().lock()->GetActions();
...
void Desensitizer::DesensitizeAction( motk::ActionPtr action )
{
	if( action )
	{
		...
		//
		m_actionStates[action->get_name().c_str()] = action->get_sensitive();
		//
...
{
	motk::ActionPtr	action =
			Application::Manager::Instance().lock()->GetActionsMgr().lock()->GetAction( pair.first );
	//
	if( action )
	{
...
Manager::Manager() :
	...
	f_startImage     ( molib::moName("START"    )  ),
	f_endImage       ( molib::moName("END"      )  ),
	f_nextImage      ( molib::moName("NEXT"     )  ),
	f_jumpInImage    ( molib::moName("JUMPIN"   )  ),
	f_delayImage     ( molib::moName("DELAY"    )  ),
	f_readyImage     ( molib::moName("READY"    )  ),
	f_damageImage    ( molib::moName("DAMAGE"   )  ),
	f_addImage       ( molib::moName("ADD"      )  ),
	f_editImage      ( molib::moName("EDIT"     )  ),
	f_deleteImage    ( molib::moName("DELETE"   )  ),
	f_d20InitImage   ( molib::moName("INIT"     )  ),
	f_d20SpotImage   ( molib::moName("SPOT"     )  ),
	f_d20ListenImage ( molib::moName("LISTEN"   )  ),
	f_d20WillImage   ( molib::moName("WILL"     )  ),
	f_websiteImage   ( molib::moName("WEBSITE") ),
	f_menubarId	(0),
	f_mainToolbarId	(0),
...
void Manager::ActivateAction( const molib::moWCString& name )
{
	motk::ActionPtr action = f_refActionGroup->get_action( name.c_str() );
	action->activate();
}


bool Manager::ToggleAction( const molib::moWCString& name )
{
	motk::ToggleActionPtr action = motk::ToggleActionPtr::cast_dynamic( f_refActionGroup->get_action( name.c_str() ) );
	if( action )
	{
		action->toggled();
	}
...
motk::ActionPtr Manager::GetAction( const molib::moWCString& name )
{
...
void Manager::AddSoftActions()
{
	...
	for( auto statPair : map )
	{
		...
		if( addAction )
		{
			molib::moWCString	name	= stat->name();
			molib::moWCString	event	= "Roll::" + name;
			molib::moWCString	accel	= stat->accel();
#ifdef DEBUG
			...
			if( !GetAction( name ) )
			{
				AddAction( name.c_str(), event.c_str(), *button_iter, accel.c_str() );
			}
			//
...
void Manager::PurgeDeletedActions()
{
	...
	for( auto statPair : map )
	{
		...
		if( stat->deleted() )
		{
			...
			const molib::moWCString	event	= "Roll::" + name;
			//
			RemoveAction( event.c_str() );
		}
	};
}
...
void Manager::CreateActions()
{
	...
	AddAction( gettext("Turn Watcher _Home Page"),
					      "Help::HomePage",      websiteImage,       "<control><shift>t" );
...
Glib::ustring Manager::GetSoftMenus( const bool toolitem )
{
	...
	for( auto statPair : statMap )
	{
		...
		{
			Glib::ustring action = Glib::ustring("Roll::") + Glib::ustring(stat->name().c_str());
			//
			if( toolitem )
			{
