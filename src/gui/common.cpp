...
molib::moWCString GetFullPathnameForImage( const molib::moWCString& basename )
{
	...
	filename = molib::moFile::FindFile(
		".:src/pixmaps:pixmaps:dev/images:../dev/images"
			":../share/turnwatcher/images:../../../../share/turnwatcher/images",
		basename, molib::moFile::MO_ACCESS_READ);
	if(!filename.IsEmpty()) {
		return filename;
	}
	...
	molib::moWCString path = application->GetApplicationPath();
	path = path.FilenameChild("images").FilenameChild(basename);

	if( molib::moFile::Access( path, molib::moFile::MO_ACCESS_READ ) )
	{
		return path;
	}
...
molib::moWCString GetFullPathnameForDoc( const molib::moWCString& basename )
{
	...
	molib::moWCString filename = "../doc/en";
	...
	molib::moApplicationSPtr application = molib::moApplication::Instance();
	molib::moWCString path = application->GetApplicationPath(true);
...
namespace
{
	...
	//
	molib::moWCString FindExec( const molib::moWCString& path, const char **software )
	{
		molib::moWCString	fullname;

		while(*software != NULL)
		{
			fullname = molib::moFile::FindFile(path, *software, molib::moFile::MO_ACCESS_EXECUTE);
			if(!fullname.IsEmpty())
			{
				break;
			}
...
void ShowDocumentation( const char *index )
{
	...
	{
		molib::moApplication::Pointer app = molib::moApplication::Instance();
		molib::moWCString path = app->GetApplicationPath();
		molib::moWCString html = path.FilenameChild("doc/en").FilenameChild(index);
	...
	if( fork() == 0 )
	{
		...
		molib::moWCString path("/usr/bin:/usr/local/bin:../../../../bin:../../../../..:");
		path += getenv("PATH");

		// TODO: should we report to the user that we could not find
		// any browser to start?!
		molib::moWCString browser = FindExec( path, software );
#ifdef DEBUG
		...
		if(!browser.IsEmpty()) {
			if(strncmp(index, "http:", 5) == 0)
			{
				...
				molib::moWCString html = Common::GetFullPathnameForDoc(index);
				if(!html.IsEmpty()) {
					if(html[0] != '/') {
						html = molib::moFile::FullPath(html);
					}
					html = "file://" + html;
#ifdef DEBUG
