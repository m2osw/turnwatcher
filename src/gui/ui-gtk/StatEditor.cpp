...
void StatEditor::CopyStats()
{
	...
	for( auto pair : GetStatMgr().lock()->GetStats() )
	{
		...
		{
			Attribute::Stat::pointer_t new_stat( new Attribute::Stat );
			new_stat->Copy( stat );
...
void StatEditor::AddStat( Attribute::Stat::pointer_t stat )
{
	...
	{
		const QString accel_key( row[f_accelKeyColumns.f_name] );
		//
		if( accel_key != stat->accel() )
		{
...
void StatEditor::AddDieFace( const QString& value )
{
...
void StatEditor::AddAccelKey( const QString& value )
{
...
Attribute::Stat::pointer_t StatEditor::GetOrCreateStat( const Gtk::TreeModel::Row& row )
{
	Attribute::Stat::pointer_t stat = row[f_columns.f_stat];
	//
	if( !stat )
	{
...
void StatEditor::OnOrderEdited( const QString& path, const QString& new_text )
{
	assert(f_store);
	Gtk::TreeModel::iterator	iter	= f_store->get_iter( path );
	const Gtk::TreeModel::Row& 	row		= *(iter);
	//
	Attribute::Stat::pointer_t stat = GetOrCreateStat( row );
	assert(stat);
	//
...
void StatEditor::OnNameEdited( const QString& path, const QString& new_text )
{
	assert(f_store);
	Gtk::TreeModel::iterator	iter	= f_store->get_iter( path );
	const Gtk::TreeModel::Row& 	row		= *(iter);
...
void StatEditor::OnDieFacesEdited( const QString& path, const QString& new_text )
{
	assert(f_store);
	Gtk::TreeModel::iterator	iter ( f_store->get_iter( path ));
	const Gtk::TreeModel::Row& 	row	 ( *(iter) );
...
void StatEditor::OnAccelKeyEdited( const QString& path, const QString& new_text )
{
	assert(f_store);
	Gtk::TreeModel::iterator	iter	( f_store->get_iter( path ) );
	const Gtk::TreeModel::Row& 	row		( *(iter) );
...
void StatEditor::OnShowMonsterOnHUDToggled( const QString& path )
{
	assert(f_store);
	Gtk::TreeModel::iterator	iter		( f_store->get_iter( path ) );
	const Gtk::TreeModel::Row& 	row			( *(iter) );
...
void StatEditor::OnShowOnToolbarToggled( const QString& path )
{
	assert(f_store);
	Gtk::TreeModel::iterator	iter		= f_store->get_iter( path );
	const Gtk::TreeModel::Row& 	row			= *(iter);
...
void StatEditor::OnShowOnHUDToggled( const QString& path )
{
	assert(f_store);
	Gtk::TreeModel::iterator	iter		( f_store->get_iter( path ) );
	const Gtk::TreeModel::Row& 	row			( *(iter) );
...
bool StatEditor::PopulateRow( const Gtk::TreeModel::iterator& iter )
{
	...
	row[f_columns.f_name]				= stat->name();
	...
	if( stat->showOnToolbar() )
	{
		...
		row[f_columns.f_accelKey]		= stat->accel();
	}
	else
	{
...
void StatEditor::Apply()
{
	...
	for( auto copy_stat : f_statsCopy )
	{
		...
		if( !found )
		{
			...
			//
			copy_stat->id( moName( copy_stat->name().c_str() ) );
...
void StatEditor::AddNewStat()
{
	...
	FixOrderings();
	//
	Attribute::Stat::pointer_t	new_stat( GetOrCreateStat( row ) );
	//
...
{
	typedef std::map<int,Attribute::Stat::pointer_t> OrderStatMap;
	...
	for( auto path : get_selection()->get_selected_rows() )
	{
		...
		//
		Attribute::Stat::pointer_t swap_stat = orderMap[new_order];
