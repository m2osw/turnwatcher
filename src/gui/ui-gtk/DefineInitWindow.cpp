...
namespace
{
    // Widget ids
    //
    const QString	InitDiceInfo("InitDiceInfo");
}


DefineInitWindow::DefineInitWindow( QString title, Gtk::Window& parent )
...
namespace
{
	QString GetText( LabelTable& tbl, const molib::moName& id )
	{
		Gtk::Entry* e;
		tbl.GetItem( id, e );
	...
	void SetText( LabelTable& tbl, const molib::moName& id, const QString& text )
	{
		Gtk::Entry* e;
		tbl.GetItem( id, e );
	...
	QString DiceFacesModifierToStr(int dice, int faces, int modifier)
	{
		...
		if(modifier == 0) {
			return QString::Format("%dd%d", dice, faces);
		}
		else {
			return QString::Format("%dd%d%+d", dice, faces, modifier);
		}
	}
...
{
	QString dice_info(Common::DiceFacesModifierToStr(
...
// this does NOT work in the demo!
	...
	QString dice_info( GetText( f_baseTable, InitDiceInfo ).c_str(), -1, molib::mowc::MO_ENCODING_UTF8 );
