...
EffectEditor::EffectEditor( const QString& title ) :
...
void EffectEditor::OnAcceptButton()
{
	...
	if( effect->name() == "" )
	{
		QString message = gettext("You must type a name for the effect!");
...
void EffectEditor::SetEffect( Effect::pointer_t effect )
{
	f_effect = effect;
...
{
	const QString	name		= GetText  ( f_table, Name );
	const QString	desc		= GetText  ( f_table, Description );
