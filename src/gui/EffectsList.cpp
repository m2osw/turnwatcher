...
void EffectsList::UpdateRow( Gtk::TreeModel::Row& row, const Effect::Pointer effect )
{
...
void EffectsList::AddEntry( const Effect::Pointer effect )
{
	assert(effect);
...
bool EffectsList::FindDuplicates( const Effect::Pointer effect )
{
	assert(effect);
	assert(f_store);
	...
	{
		Effect::Pointer	row_effect = row[f_columns.f_effect];
		assert(row_effect);
...
void EffectsList::AddEffects( Combatant::Character::Pointer ch )
{
	ClearEffects();
...
void EffectsList::GetEffects( Combatant::Character::Pointer ch )
{
	assert(ch);
	assert(f_store);
	Effect::List effect_list;
	for( auto row : f_store->children() )
	{
		Effect::Pointer	effect( row[f_columns.f_effect] );
...
void EffectsList::UpdateEffect( Effects::Effect::Pointer effect_to_find )
{
...
Effects::Effect::Pointer EffectsList::GetFirstSelected()
{
	assert(f_store);
	SelectedPaths            selected ( f_selection->get_selected_rows() );
	Gtk::TreeModel::Path     path     ( *(selected.begin())              );
	Gtk::TreeModel::Row      row      ( *(f_store->get_iter(path))       );
	Effects::Effect::Pointer effect   ( row[f_columns.f_effect]          );
	assert(effect);
	return effect;
}
