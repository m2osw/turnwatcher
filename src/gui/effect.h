...
{
public:
	typedef std::shared_ptr<Effect>	Pointer;
	typedef std::vector<Pointer>	List;

	Effect();

	molib::moWCString	name()	const					{ return f_name; }
	void				name( const molib::moWCString& val )		{ f_name = val;  }

	molib::moWCString	description() const				{ return f_description; }
	void				description( const molib::moWCString& val )	{ f_description = val;  }
	...
	void			Copy( Effect::Pointer effect );
...
private:
	...
	molib::moWCString	f_description;
	int			f_totalRounds;
	int			f_roundsUsed;
